...
def process_fastq(fastq_1, fastq_2, threads, output, output_name, config):
    ...
    qc_command = (
        ...
        f"--compression {compression_level} --dup_calc_accuracy {dup_calc_accuracy} "
        f"--length_required {length_required} --html {output}/{output_name}.html"
    )
...
def process_bam_to_fastq(
    ...
    Args:
        ...
        threads (int): Number of threads to use.
        config (dict): Configuration dictionary containing tool paths and parameters.
        reference_assembly (str, optional): Reference assembly used ("hg19" or "hg38"). Defaults to "hg19".
        fast_mode (bool, optional): If True, skips filtering of unmapped and partially mapped reads. Defaults to False.
        delete_intermediates (bool, optional): If True, deletes intermediate files after processing. Defaults to True.
        keep_intermediates (bool, optional): If True, keeps intermediate files for later use. Defaults to False.
    ...
    if not fast_mode:
        ...
        command_merge = (
            ...
            f"{output}/{output_name}_unmapped2.bam {output}/{output_name}_unmapped3.bam"
        )
    ...
    if keep_intermediates and all(p.exists() for p in [final_fastq_1, final_fastq_2, final_fastq_other, final_fastq_single]):
        logging.info(
        ...
        command_sort_fastq = (
            ...
            f"-2 {final_fastq_2} -0 {final_fastq_other} -s {final_fastq_single}"
        )
    ...
    return str(final_fastq_1), str(final_fastq_2), str(final_fastq_other), str(final_fastq_single)
