...
def _advntr_baseline(
    run: TargetRun, required_callers: tuple[str, ...]
) -> tuple[tuple[dict[str, object], ...], bool, bytes | None]:
    ...
    for policy_id, policy in policies.items():
        for member in roster.members:
            ...
            if run.execution_kind == "scalar-replay":
                calls, assessable = (
                    _advntr_replay(run, cast(bytes, capture_raw)) if capture_raw is not None else ((), True)
                )
                row = replayed_caller_observation(
                    ...
                    policy=policy,
                    capture_policy_sha256=run.capture_policy_sha256,
                    advntr_calls=calls,
                    advntr_assessable=assessable,
                ...
                baseline_row = replayed_caller_observation(
                    ...
                    truth,
                    kestrel_capture=capture,
                    policy=protocol.baseline_policy,
                    capture_policy_sha256=run.capture_policy_sha256,
                    advntr_calls=baseline_calls,
                    advntr_assessable=baseline_assessable,
                    source_evidence_sha256=source_sha,
                )
    ...
    draft = decode_caller_role_evidence(
        {
            ...
            "eligible_roster_sha256": roster.sha256,
            "run_manifest_sha256": "0" * 64,
            "baseline_assets_sha256": "0" * 64,
...
def fit_caller_bundle(args: object, output: Path) -> bool:
    ...
    if needs_background:
        ...
        training_background = fit_caller_training_background(
            ...
            output=(output / "training-background").resolve(),
        )
...
def evaluate_fixed_caller_source(
    ...
    *,
    ...
    evidence = load_caller_source_evidence(
        ...
        source,
        receipt,
        phase=cast(EvaluationPhase, source.role),
    ...
    result = evaluate_caller_grid(protocol, roster, evidence, fixed_candidate_id=profile.selected_protocol_candidate_id)
    return result, render_caller_evaluation(result, protocol=protocol, roster=roster, evidence=evidence)
