...
def filter_vcf(input_path, output_path):
    """
    Filters a VCF file to extract indels (insertions and deletions) and writes them to a new file.

    An indel is a type of variant where either the reference allele or the alternate allele
...
def filter_indel_vcf(indel_vcf, output_ins, output_del):
    """
    Further filters the indel VCF file to separate insertions and deletions into two separate files.

    Insertions and deletions are differentiated based on the length of the reference allele (REF) and the alternate allele (ALT):
    ...
    with open(indel_vcf, "r") as vcf_file, open(output_ins, "w") as insertion_file, open(output_del, "w") as deletion_file:
        for line in vcf_file:
            if line.startswith("##"):
                ...
                # The header line that starts with "#CHROM" is written to both output files
                insertion_file.write(line)
                deletion_file.write(line)
            else:
                # Split the line into columns and extract the REF and ALT columns
                _, _, _, ref, alt, *_ = line.split("\t")
