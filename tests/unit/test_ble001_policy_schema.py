...
def test_live_ble001_diagnostics_match_reviewed_handler_counts() -> None:
    ...
    assert policy.expected_identities == 75
    assert policy.expected_categories == (28, 16, 31)
    assert len(policy.handlers) == 75
    assert Counter(row.category for row in policy.handlers) == Counter({"A": 28, "B": 16, "C": 31})
...
def test_live_policy_cli_reports_success_after_every_behavior_node_resolves() -> None:
    ...
    assert "categories A/B/C: 53/16/35" in result.stdout
