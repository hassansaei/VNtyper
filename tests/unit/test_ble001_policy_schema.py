...
def test_live_ble001_diagnostics_match_reviewed_handler_counts() -> None:
    ...
    assert policy.expected_normal == 97
    assert policy.expected_all == 104
    assert policy.expected_all - policy.expected_normal == 7
    assert policy.expected_identities == 76
    assert policy.expected_categories == (28, 16, 32)
    assert len(policy.handlers) == 76
    assert Counter(row.category for row in policy.handlers) == Counter({"A": 28, "B": 16, "C": 32})
    assert sum(row.normal_count for row in policy.handlers) == 97
    assert sum(row.all_count for row in policy.handlers) == 104
...
def test_live_policy_cli_reports_success_after_every_behavior_node_resolves() -> None:
    ...
    assert "categories A/B/C: 52/16/36" in result.stdout
