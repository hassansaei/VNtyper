...
def test_live_ble001_diagnostics_match_reviewed_handler_counts() -> None:
    ...
    assert policy.expected_normal == 94
    assert policy.expected_all == 101
    assert policy.expected_all - policy.expected_normal == 7
    assert policy.expected_identities == 72
    assert policy.expected_categories == (28, 16, 28)
    assert len(policy.handlers) == 72
    assert Counter(row.category for row in policy.handlers) == Counter({"A": 28, "B": 16, "C": 28})
    assert sum(row.normal_count for row in policy.handlers) == 94
    assert sum(row.all_count for row in policy.handlers) == 101
...
def test_live_policy_cli_reports_success_after_every_behavior_node_resolves() -> None:
    ...
    assert "categories A/B/C: 53/16/32" in result.stdout
