...
from vntyper.scripts.nomenclature_annotate import annotate_advntr_frame, annotate_kestrel_frame
...
def test_identity_translation_diagnostic_columns_have_exact_order_and_spelling() -> None:
    assert IDENTITY_TRANSLATION_DIAGNOSTIC_COLUMNS == (
        "Molecular_Identity",
...
def test_kestrel_pre_result_has_only_the_four_approved_identity_diagnostics(tmp_path: Path) -> None:
    ...
    identity_facing = tuple(column for column in pre_result.columns if column.startswith("Molecular_Identity"))
    internal_codec = tuple(column for column in pre_result.columns if column.startswith("__Identity_"))
    assert identity_facing == IDENTITY_TRANSLATION_DIAGNOSTIC_COLUMNS
    assert internal_codec == IDENTITY_CAPTURE_COLUMNS
...
def test_positive_advntr_publication_counts_two_distinct_identities_on_both_rows(tmp_path: Path) -> None:
    ...
    source.write_text(
        "#VID\tState\tNumberOfSupportingReads\tMeanCoverage\tPvalue\n"
        "25561\tI22_2_G_LEN1\t11\t153.98\t0.0001\n"
        "25561\tI22_2_A_LEN1\t12\t154.98\t0.0002\n",
        encoding="utf-8",
    )
    ...
    result = pd.read_csv(tmp_path / "output_adVNTR_result.tsv", sep="\t", keep_default_na=False)
    assert result["Variant"].tolist() == ["I22_2_G_LEN1", "I22_2_A_LEN1"]
    assert result["Molecular_Identity"].tolist() == [
        "MUC1-X-60-coding-v1|60|59|-|C",
        "MUC1-X-60-coding-v1|60|59|-|T",
    ]
    assert result["Molecular_Identity_Status"].tolist() == [
        "legacy-selected-among-multiple",
        "legacy-selected-among-multiple",
    ]
    assert result["Identity_Hypothesis_Count"].tolist() == [2, 2]


def test_positive_advntr_row_with_exact_absent_context_sentinel_remains_publishable(tmp_path: Path) -> None:
    source = tmp_path / "output_adVNTR.vcf"
    source.write_text(
        "#VID\tState\tNumberOfSupportingReads\tMeanCoverage\tPvalue\n25561\tBANANA_LEN1\t11\t153.98\t0.0001\n",
        encoding="utf-8",
    )
    ...
    result = pd.read_csv(tmp_path / "output_adVNTR_result.tsv", sep="\t", keep_default_na=False)
    assert result["Variant"].tolist() == ["BANANA_LEN1"]
    assert result.loc[0, "RU"] == "."
    assert result.loc[0, "POS"] == "."
    assert result.loc[0, list(IDENTITY_COLUMNS)].to_dict() == {
        "Molecular_Identity": "",
        "Molecular_Identity_Status": "unresolved",
        "Equivalent_Representation_Count": 0,
        "Identity_Hypothesis_Count": 0,
...
def test_exact_absent_context_sentinel_constructs_typed_none_pair() -> None:
    observed: list[AdvntrRepresentation] = []

    class RecordingTranslator:
        def translate_kestrel(self, representation: KestrelRepresentation) -> IdentityTranslation:
            raise AssertionError(f"Unexpected Kestrel translation: {representation}")

        def translate_advntr(self, representation: AdvntrRepresentation) -> IdentityTranslation:
            observed.append(representation)
            return _UNRESOLVED

    cells = advntr_identity_result_rows(
        ({"Variant": "BANANA_LEN1", "RU": ".", "POS": "."},),
        RecordingTranslator(),
    )

    assert observed == [AdvntrRepresentation("BANANA_LEN1", None, None)]
    assert cells == [
        {
            "Molecular_Identity": "",
            "Molecular_Identity_Status": "unresolved",
...
@pytest.mark.parametrize(
    ("repeat_units", "positions", "message"),
    [
        (".", "22", "must both use the absent-context sentinel"),
        ("2", ".", "must both use the absent-context sentinel"),
        ("2", "banana", "POS must contain comma-separated positive integers"),
    ],
)
def test_advntr_publication_rejects_mismatched_or_malformed_context(
    repeat_units: str,
    positions: str,
    message: str,
) -> None:
    component = translation_component_from_config(load_nomenclature_config())
    row = {
    ...
    with pytest.raises(ValueError, match=message):
        advntr_identity_result_rows((row,), component)


def test_moved_negative_row_predicate_is_directly_callable() -> None:
    from vntyper.scripts.nomenclature_frame_presentation import is_negative_result_row

    assert is_negative_result_row(pd.Series({"Confidence": "Negative"})) is True
    assert is_negative_result_row(pd.Series({"VID": "Negative"})) is True
    assert is_negative_result_row(pd.Series({"Motif": "None"})) is True
    assert is_negative_result_row(pd.Series({"VID": "25561", "Variant": "I22_2_G_LEN1"})) is False


def test_moved_nomenclature_cell_projection_is_directly_callable() -> None:
    from vntyper.scripts.nomenclature import from_advntr, reconcile, render
    from vntyper.scripts.nomenclature_frame_presentation import nomenclature_result_cells

    merged = reconcile(*from_advntr("I22_2_G_LEN1"), support=24)

    assert nomenclature_result_cells(merged, advntr=render(merged)) == {
        "Nomenclature": "59dupC",
        "Nomenclature_Tier": "B",
...
def test_moved_advntr_frame_adapter_is_the_compatibility_export() -> None:
    from vntyper.scripts.nomenclature_frame_presentation import annotate_advntr_frame as moved_adapter

    frame = pd.DataFrame([{"VID": "25561", "Variant": "I22_2_G_LEN1", "NumberOfSupportingReads": "24"}])

    assert annotate_advntr_frame is moved_adapter
    assert moved_adapter(frame).loc[0, "Nomenclature_adVNTR"] == "59dupC"


def test_moved_advntr_frame_adapter_preserves_invalid_support_as_unknown() -> None:
    from vntyper.scripts.nomenclature_frame_presentation import annotate_advntr_frame as moved_adapter

    frame = pd.DataFrame([{"VID": "25561", "Variant": "I22_2_G_LEN1", "NumberOfSupportingReads": "not-a-number"}])

    assert moved_adapter(frame).loc[0, "Nomenclature_Tier"] == "B"


def test_absent_advntr_context_still_requires_one_strict_state_field() -> None:
    component = translation_component_from_config(load_nomenclature_config())
    row = {
        "State": "BANANA_LEN1",
        "Variant": "BANANA_LEN1",
    ...
    with pytest.raises(ValueError, match="exactly one non-empty State or Variant"):
        advntr_identity_result_rows((row,), component)
