...
def test_filter_by_alt_values_gg_filter_below_threshold(kestrel_config_mock):
    ...
    df = pd.DataFrame(
        {
            ...
            "Depth_Score": [
                ...
                0.02,
                0.5,
            ],  # 0.019 < threshold => remove, 0.02 >= threshold => keep
        }
    )
    ...
    assert (
        out["Depth_Score"] < 0.02
    ).sum() == 0, "No row should have Depth_Score < 0.02 for 'GG' alt."
...
def test_filter_by_alt_values_drop_left_right(kestrel_config_mock):
    ...
    assert (
        "left" not in out.columns and "right" not in out.columns
