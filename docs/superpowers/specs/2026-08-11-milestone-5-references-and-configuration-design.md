...
```jsonc
{
  ...
  "sources": {
    "hg19":         { "url": "...goldenPath/hg19/chromosomes/chr1.fa.gz", "source_sha256": "..." },
    "hg38":         { "url": "...goldenPath/hg38/chromosomes/chr1.fa.gz", "source_sha256": "..." },
    "GRCh37":       { "url": "...GCF_000001405.25_GRCh37.p13...",         "source_sha256": "..." },
    "GRCh38":       { "url": "...GCF_000001405.40_GRCh38.p14...",         "source_sha256": "..." },
    "hg19_ensembl": { "url": ".../pub/grch37/release-116/fasta/...",      "source_sha256": "..." },
    "hg38_ensembl": { "url": ".../pub/release-116/fasta/...",             "source_sha256": "..." }
  },
  "seeds": {
...
Three things about this shape are enforced by code, not conventions:

- **`sources` is keyed by physical reference id** — `hg19`, `hg38`, `GRCh37`, `GRCh38`,
  `hg19_ensembl`, `hg38_ensembl` — and each entry spells its digest **`source_sha256`**.
  Those are the names `install_references.resolve_source_location` reads;
  `bundle_release.spec_source` rejects a spec that keys by source-prefixed names or spells
  the field `sha256`, because either would be silently ignored by the installer.
- **The spec may not contradict `install_references_config.json`.** The trust anchor lives
  in VNtyper, not next to the assets, so `bundle_release.cross_check_sources` and
  `resolve_source_location` both fail on any disagreement in `url` or `source_sha256`. The
  spec may fill in a field the config leaves blank; it may not overrule one. Moving a
  future release to a newer upstream therefore takes a reviewed VNtyper commit — which is
  a base-image content-hash change anyway.
- **`seeds` pins exactly the four files above and nothing else** — `MUC1_motifs_Rev_com.fa`,
  `code-adVNTR_RUs.fa`, `vntr_db_advntr.zip`, `filter_config.json`. The two `.fai` files and
  `generate_vntr_reference.py` are deliberately *not* pinned: the workflow does not stage
  the `.fai` files, because the build regenerates them with its own samtools, so pinning
  them would compare regenerated bytes against a committed digest as a fatal check, three
  hours in, over a samtools formatting difference.
