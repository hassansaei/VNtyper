...
| `--derive-only` | flag | off | Rebuild only the derived reference files (`muc1_region_hg19.fa`, `muc1_region_hg38.fa`, `All_Pairwise_and_Self_Merged_MUC1_motifs_filtered.fa`) from genomes and seeds already installed. Downloads nothing; each output is verified against its committed checksum. Cannot be combined with `--from-source` |
