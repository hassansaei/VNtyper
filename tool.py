...
if None not in (fastq_1, fastq_2):
    QC_command =  "fastp "+ "--thread " + args.threads + " " + "--in1 " + fastq_1 + " --in2 " + fastq_2 + " --out1 " + args.working_dir + args.output + "/kmer_method/" + args.output + "_R1.fastq.gz" + " " + "--out2 " + args.working_dir + args.output + "/kmer_method/" + args.output + "_R2.fastq.gz" + " --phred64 --compression 6 --disable_adapter_trimming --dedup --dup_calc_accuracy 3 --length_required 120 --html " + output + "temp/" + args.output + ".html"
...
if vcf_path.is_file():
    ...
    if None not in (fastq_1, fastq_2, reference_VNTR):
        kmer_command =  "kestrel" +  "-k " +  args.Kmer + " -r " + reference_VNTR  + " -o " + vcf_out + " " + fastq_1 + " " + fastq_2 
...
if None not in (sorted_bam , reference):
    adVNTR_command = "advntr genotype -fs -vid 25561 --outfmt vcf --alignment_file " + sorted_bam + " -o " + output + "adVNTR/" + args.output + ".vcf" + " " + "-m " + db_file_hg19 + " -r " + args.reference_file + " --working_directory " + output + "adVNTR/"
