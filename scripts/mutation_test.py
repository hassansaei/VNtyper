#!/usr/bin/env python3
"""
Advisory mutation scoring for VNtyper's decision-heavy modules.
...
#:
#: The first six are the pure-decision modules: they turn variants and motifs into the
...
TARGETS: dict[str, tuple[str, ...]] = {
    ...
    # fuses I/O with the decisions -- but it owns select_single_best_variant and the
    # kestrel_result.tsv publication tail: the code that picks and publishes the
    # reported call. List built with `grep -rln kestrel_genotyping tests/unit/`, per the
    # WIDE rule above.
    "vntyper/scripts/kestrel_genotyping.py": (
        "tests/unit/test_bcftools_optional.py",
        "tests/unit/test_builders.py",
        "tests/unit/test_cli_handlers.py",
        "tests/unit/test_confidence_boundaries.py",
        "tests/unit/test_generate_bed_file.py",
        "tests/unit/test_generate_report.py",
        "tests/unit/test_haplo_count_and_selection.py",
        "tests/unit/test_helpers.py",
        "tests/unit/test_kestrel_command_extraction.py",
        "tests/unit/test_kestrel_derived_vcf_guard.py",
        "tests/unit/test_kestrel_filtering.py",
        "tests/unit/test_kestrel_result_publication.py",
        "tests/unit/test_kestrel_split_counting.py",
        "tests/unit/test_kestrel_subthreshold_note.py",
        "tests/unit/test_motif_decisions.py",
        "tests/unit/test_motif_exclusion_pipeline.py",
        "tests/unit/test_motif_preprocessing.py",
...
def format_markdown(results: list[ModuleResult], elapsed: float) -> str:
    ...
    out.append(f"    modules then, {total} over {len(results)} modules now, generated by a different operator set.")
