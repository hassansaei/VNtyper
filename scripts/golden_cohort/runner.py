...
def pipeline_argv(
    ...
    *,
    ...
    Args:
        ...
        advntr_case_threads: ``--threads`` for a case that also runs adVNTR. The wider value
            reaches alignment, samtools and fastp; **adVNTR itself never sees it** and runs at
            the ``-t`` its own configuration sets, which is 1. The name says "case", not
            "adVNTR", for exactly that reason -- it was ``advntr_threads``, which read as the
            thread count adVNTR received and was never true (#247).
    ...
    argv.extend(
        [
            ...
            str(output_dir),
            "--threads",
            str(advntr_case_threads if case.get("advntr") else threads),
        ]
    )
    ...
    *,
    ...
    Args:
        ...
        advntr_case_threads: ``--threads`` for cases that also run adVNTR. Reaches alignment,
            samtools and fastp only; adVNTR runs at ``-t 1`` from its own configuration (#247).
    ...
    def launch_case(case: dict[str, Any]) -> tuple[str, dict[str, Any]]:
        ...
        argv = pipeline_argv(
            ...
            output_dir,
            threads=threads,
            advntr_case_threads=advntr_case_threads,
