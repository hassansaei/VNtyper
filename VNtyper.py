...
welcomeMessage = """
======================================================================
Genotyping MUC1 coding-VNTR using mapping-free genotyping (Kestrel) and profile-HMM-based (code-adVNTR) algorithms
...
endMessage = """
==============================
Thanks for using MUC1 pipeline!
==============================
"""
...
if None not in (fastq_1, fastq_2):
    QC_command = tools_path + "./fastp "+ "--thread " + args.threads + " " + "--in1 " + fastq_1 + " --in2 " + fastq_2 + " --out1 " + args.working_dir + args.output + "/kmer_method/" + args.output + "_R1.fastq.gz" + " " + "--out2 " + args.working_dir + args.output + "/kmer_method/" + args.output + "_R2.fastq.gz" + " --compression 6 --disable_adapter_trimming --dedup --dup_calc_accuracy 3 --length_required 60 --html " + output + "temp/" + args.output + ".html"
...
vcf_out = args.working_dir + args.output + "/kmer_method/" + args.output + ".vcf"
...
# Kmer mapping-free genotyping out processing
...
def final_processing(df):
    ...
    df3["Frame_Score"] = (df3.alt_len - df3.ref_len) / 3
    ...
    df3.drop(['TrueFalse', 'ref_len', 'alt_len'], axis=1, inplace=True)
...
if IG.empty:
    ...
    IG.to_csv(args.working_dir + args.output + "/adVNTR/" + args.output + 'I_G.bed', sep='\t', index=False)
    df_.to_csv(args.working_dir + args.output + "/adVNTR/"+ args.output+'_advntr.bed', sep='\t', index=False)
...
logging.info('Results saved to the working directory')
