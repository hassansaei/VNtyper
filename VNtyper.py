...
if args.bam:
    ...
    fastq_2 = output + args.output + "_R1.fastq.gz"
...
kmer_command_20 = "java -Xmx15g -jar " + "/usr/local/lib/kestrel-1.0.1/kestrel.jar -k " +  " 20 " + " -r " + reference_VNTR  + " -o " + vcf_out + "  " + fastq_1 + " " + fastq_2 + " " + " --temploc " + args.working_dir + args.output + "/temp/" 
...
if vcf_path.is_file():
    ...
    if None not in (fastq_1, fastq_2, reference_VNTR):
        print ("Launching Kestrel...with Different Kmer sizes!\n")
...
def conditions(Kestrel_concat):
    if (Kestrel_concat['Depth_Score'] <= 0.00464) or (Kestrel_concat['Estimated_Depth_Variant_ActiveRegion'] <= 200):
        return 'Red_Zone'
    elif (Kestrel_concat['Estimated_Depth_AlternateVariant'] > 20) and (Kestrel_concat['Depth_Score'] >= 0.00464) and (Kestrel_concat['Depth_Score'] <= 0.005):
    ...
    elif (Kestrel_concat['Estimated_Depth_AlternateVariant'] > 20) and (Kestrel_concat['Estimated_Depth_AlternateVariant'] < 100) and (Kestrel_concat['Depth_Score'] >= 0.005):
        return 'High_Precision'
    elif (Kestrel_concat['Estimated_Depth_AlternateVariant'] > 100) and (Kestrel_concat['Depth_Score'] >= 0.005):
        return 'High_Precision*'
...
def process_kmer(Kmer_A):
    if Kmer_A.empty:
        ...
        Kestrel_concat = Kestrel_concat[Kestrel_concat['Depth_Score'] >= 0.00464]
...
columns_kmer = ['Motif', 'Variant', 'POS', 'REF', 'ALT', 'Motif_sequence (RevCom)', 'Estimated_Depth_AlternateVariant', 'Estimated_Depth_Variant_ActiveRegion', 'Depth_Score', 'Confidence']
